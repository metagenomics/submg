...
def __check_coverage(arguments: dict,
    ...
    #     assembly_data = {}
    # bin_data = utility.from_config(config, 'BINS')
    # if 'COVERAGE_VALUE' in assembly_data.keys() and 'COVERAGE_FILE' in bin_data.keys():
    #     if 'BAM_FILES' in config.keys():


    # if not arguments['submit_assembly'] and not arguments['submit_bins'] and not arguments['submit_mags']:
    #     return
    # if arguments['known_coverage']:
    #     utility.from_config(config, 'ASSEMBLY', 'COVERAGE_VALUE')
    ...
    #             loggingC.message(err, threshold=-1)
    #             exit(1)
    # elif arguments['coverage_from_bam']:
    #     bam_files = utility.from_config(config, 'BAM_FILES')
    #     if not isinstance(bam_files, list):
    #         bam_files = [bam_files]
    #     if len(bam_files) == 0:
    #         err = f"\nERROR: You chose to submit an assembly, bins or MAGs, but did not provide any BAM files."
    #         loggingC.message(err, threshold=-1)
    #         exit(1)
    #     for bam_file in bam_files:
    #         if not os.path.isfile(bam_file):
    ...
    #             exit(1)
    # else:
    #     err = f"\nERROR: You chose to submit an assembly, bins or MAGs, but did not provide any coverage data."
    #     loggingC.message(err, threshold=-1)
    #     exit(1)
